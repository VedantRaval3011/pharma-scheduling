// /api/admin/detector-type/audit/route.ts
import { NextRequest, NextResponse } from 'next/server';
import connectDB from '@/lib/db';
import DetectorTypeAuditLog from '@/models/detectorTypeAudit'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const auditLog = new DetectorTypeAuditLog({
      userId,
      action,
      data,
      previousData,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const detectorType = searchParams.get('detectorType');
    ...
    // Filter by detectorType (exact match in data.detectorType or previousData.detectorType)
    if (detectorType) {
      query.$or = [
        { 'data.detectorType': detectorType },
        { 'previousData.detectorType': detectorType }
      ];
    }
    ...
    // Search by keyword in detectorType or description
    if (searchTerm) {
      query.$or = query.$or || [];
      query.$or.push(
        { 'data.detectorType': { $regex: searchTerm, $options: 'i' } },
        { 'data.description': { $regex: searchTerm, $options: 'i' } },
        { 'previousData.detectorType': { $regex: searchTerm, $options: 'i' } },
        { 'previousData.description': { $regex: searchTerm, $options: 'i' } }
      );
    }

    const auditLogs = await DetectorTypeAuditLog.find(query).sort({ timestamp: -1 });
