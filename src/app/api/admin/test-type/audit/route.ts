...
import TestTypeAuditLog from '@/models/test-type-audit';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const auditLog = new TestTypeAuditLog({
      userId,
      action,
      data,
      previousData,
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Filter by testType (exact match in data.testType or previousData.testType)
    if (testType) {
      query.$or = [
        { 'data.testType': testType },
        { 'previousData.testType': testType }
      ];
    }
    ...
    // Search by keyword in testType or description
    if (searchTerm) {
      query.$or = query.$or || [];
      query.$or.push(
        { 'data.testType': { $regex: searchTerm, $options: 'i' } },
        { 'data.description': { $regex: searchTerm, $options: 'i' } },
        { 'previousData.testType': { $regex: searchTerm, $options: 'i' } },
        { 'previousData.description': { $regex: searchTerm, $options: 'i' } }
      );
    }

    const auditLogs = await TestTypeAuditLog.find(query).sort({ timestamp: -1 });
